...
Use `job::export()` to control what is returned from a job. It should be called as the last command in the job. The documentation for `job::export()` includes code examples and may be useful to read first. If you don't call `job::export()` explicitly in your code chunk, it defaults to `job::export("changed", file = NULL)`. In this article, we'll discuss situations where it definitely makes sense to deviate from this default.

Also check out the related [article on controlling imports to a job](../articles/import.html).
