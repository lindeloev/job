...
Use `job::export()` to control what is returned from a job. Here are a few examples:
...
You can also use `save()` and `save.image(envir = sys.frame(sys.nframe()))`. The latter will include two functions called `emitProgress()` and `sourceWithProgress()` which were set by RStudio.
