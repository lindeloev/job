...
job = function(..., import = "all", packages = .packages(), opts = options(), title = NULL) {
  ...
  print("Assign unnamed by position")
  ...
  if (n_unnamed > 0) {
    ...
    for (i in seq_len(n_unnamed)) {
      print(paste0("Replacing unnamed at position ", i, ": ", func_arg_names_pruned[i]))
  ...
  # PACKAGES #
  ############
  print("Get list of attached packages")
  # List currently attached Then write "library(x)" code
  if (is.character(packages) || length(packages) == 0) {
    packages_str = paste0("library(", packages, ", warn.conflicts = FALSE)", collapse = "\n")
  } else {
...
message('\n', Sys.time(), ': Job finished. Exporting results to main session...')
options(warn = -1)")
